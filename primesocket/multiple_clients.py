...
def run_multiple_clients(ip: str, port: int, num_clients: int):
    ...
    for client_id in range(1, num_clients + 1):
        process = multiprocessing.Process(target=run_client, args=(ip, port, client_id))
        process.start()
...
if __name__ == "__main__":
    """Main entry point to run the client applications."""
    server_ip = "127.0.0.1"
    server_port = 9999
    num_clients = 2
    run_multiple_clients(server_ip, server_port, num_clients)